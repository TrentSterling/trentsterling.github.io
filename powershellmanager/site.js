...
modeButton.addEventListener('click', () => {
  ...
  try { localStorage.setItem('psm-site-mode', mode); } catch {}
  labelMode();
});
...
document.querySelectorAll('[data-preview]').forEach(button => button.addEventListener('click', async () => {
  ...
  try {
    ...
    hero.src = image.src;
    hero.alt = `PowerShellManager in the ${button.textContent} theme, with anonymized terminal titles`;
...
lightbox.addEventListener('click', event => { const r=lightbox.getBoundingClientRect(); if(event.target===lightbox && (event.clientX<r.left || event.clientX>r.right || event.clientY<r.top || event.clientY>r.bottom)) lightbox.close(); });
