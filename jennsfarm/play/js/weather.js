...
function newParticle() {
    return {
        x: (Math.random() - 0.5) * SPREAD, y: Math.random() * TOP, z: (Math.random() - 0.5) * SPREAD,
...
export function updateWeather(dt, seasonName, playerPos) {
    ...
    if (w.kind !== currentKind) rebuild(w);
    mesh.visible = true;
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (p.y < 0) { p.y = TOP; p.x = (Math.random() - 0.5) * SPREAD; p.z = (Math.random() - 0.5) * SPREAD; }
        if (leaves) p.spin += p.vspin * dt;
        _p.set(px + p.x, p.y, pz + p.z);
