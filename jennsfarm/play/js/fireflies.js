...
export function updateFireflies(dt, isNight, playerPos, t) {
    ...
    if (c.kind !== currentKind) rebuild(c);
    mesh.visible = true;
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (p.x > SPREAD / 2) p.x -= SPREAD; else if (p.x < -SPREAD / 2) p.x += SPREAD;
        if (p.z > SPREAD / 2) p.z -= SPREAD; else if (p.z < -SPREAD / 2) p.z += SPREAD;
        const bob = Math.sin(t * p.sp + p.ph) * (firefly ? 0.35 : 0.22);
        _p.set(px + p.x, p.y + bob, pz + p.z);
